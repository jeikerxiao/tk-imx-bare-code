...
本目录存放I.MX6ULL裸机开发工程第六个实验——**蜂鸣器驱动**的完整代码。
该实验在BSP模块化架构基础上，新增蜂鸣器（BEEP）驱动模块，通过GPIO控制有源蜂鸣器的开关。
实验目标是让读者掌握BSP模块的扩展方法，学会新增一个外设驱动的完整流程，包括头文件定义、源文件实现、Makefile路径配置及主程序调用。
